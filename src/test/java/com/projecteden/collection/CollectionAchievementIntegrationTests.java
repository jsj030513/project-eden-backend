...
class CollectionAchievementIntegrationTests {
	...
	@Test void collectionSummarySucceeds() throws Exception {int total=RecognizedObject.values().length;collectionService.registerDiscovery(character.getId(),RecognizedObject.FLOWER);mockMvc.perform(get("/api/collections/me").header("Authorization",bearer())).andExpect(status().isOk()).andExpect(jsonPath("$.totalCollectableCount").value(total)).andExpect(jsonPath("$.uniqueCollectedCount").value(1)).andExpect(jsonPath("$.completionRate").value(100.0/total)).andExpect(jsonPath("$.collections",hasSize(1)));}
